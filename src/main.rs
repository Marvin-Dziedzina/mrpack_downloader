...
fn main() -> Result<(), &'static str> {
    ...
    info!("Deserialization of mrpack done");
    ...
    for file in mrpack.files.iter() {
        ...
        for download_link in file.downloads.iter() {
            ...
            let mut file = match File::create(&mod_path) {
                ...
                Err(e) => {
                    download_failed = true;
                    warn!("{} failed to install with error: {}", mod_name, e);
                    continue;
                }
            };
            let mod_bytes = match mod_data.bytes() {
                ...
                Err(e) => {
                    download_failed = true;
                    warn!("{} failed to get bytes with error: {}", mod_name, e);
                    continue;
                }
            };
            ...
            match file.write_all(&mod_bytes) {
                ...
                Err(e) => {
                    download_failed = true;
                    warn!("{} failed to write to drive with error: {}", mod_name, e);
                    continue;
                }
            };

            info!("{} sucessfully downloaded!", mod_name);
